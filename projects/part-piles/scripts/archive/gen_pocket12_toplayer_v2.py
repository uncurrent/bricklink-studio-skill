#!/usr/bin/env python3
"""
Pocket 12 — Top layer v2 (scattered, varied angles, almost touching)

Key improvements over v1:
1. Fully continuous random Y-rotation (no discrete 0/45/90 snapping)
2. GAP = -4 (parts visually nearly touching / slightly overlapping at AABB level)
3. Candidate selection: pick randomly from top-K closest-to-center,
   weighted so closer = more likely → organic scatter, not rigid rings
4. More angles sampled (N_ANGLES=72) for denser touch-point coverage
5. Larger palette with more repeats for denser packing
"""
import math, sys, random as _random

# ── constants ──────────────────────────────────────────────────────────────────
CX, CZ      = -100, -130
OVAL_A      = 162       # tight oval for top layer
OVAL_B      = 125
GAP         = -4        # negative: AABB touch formula allows slight visual overlap
                        # flat plates are thin (4 LDU), so AABB overlap is fine visually
Y_BASE      = 0
Y_JITTER    = 4         # ±jitter (very slight — top layer is nearly flat)
TILT_STR    = 0.05      # very flat tilt (top layer = covering tiles/plates)
N_ANGLES    = 72        # more angles = more candidate positions
N_PARTS_MAX = 36        # top layer part count
N_SEEDS     = 40
TOP_K       = 6         # pick randomly from top-K candidates (breaks ring pattern)

COLORS = [4, 1, 14, 25, 2, 6, 10, 3, 9, 11, 13, 12, 5, 15, 19, 72,
          28, 47, 18, 46]

# ── top-layer palette ─────────────────────────────────────────────────────────
# Mostly plates & tiles — flat, thin, like the real top layer of a pile
PALETTE = [
...
def make_R_flat(rng, ry_rad):
    """Nearly flat: fully random Y-rotation, tiny X/Z wobble."""
...
def touch_distance(ref_ex, ref_ez, new_ex, new_ez, ca, sa):
    """Distance in direction (ca,sa) so new part AABB just touches ref AABB."""
...
def generate(seed):
    ...
    # Keep first few big plates at front, shuffle the rest
    rng.shuffle(parts[4:])
    ...
    for pid, dims in parts:
        ...
        # FULLY CONTINUOUS random Y-rotation — any angle, not just multiples of 45°
        ...
        # Find touching positions: all placed parts × all angles
        candidates = []

        for ref in placed:
            rx0, ry0, rz0, rex, rey, rez = ref
            for angle in angles:
        ...
        # Pick randomly from TOP_K closest positions (weighted: closer = more likely)
        # This breaks the rigid ring pattern while still biasing toward center
        k = min(TOP_K, len(candidates))
        pool = candidates[:k]
        # Weights: 1/rank (rank 1 = weight 1/1, rank 2 = weight 1/2, ...)
...
# ── run ────────────────────────────────────────────────────────────────────────
print("Running sequential outward placement v2...", file=sys.stderr)
...
out = "/tmp/pocket12_toplayer_v2.ldr"
