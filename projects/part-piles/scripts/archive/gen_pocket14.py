#!/usr/bin/env python3
"""
gen_pocket14.py — Brickit Pocket 14 generation script

First "Medium" size pocket — larger top-down footprint than Small (P1–P13).
Based on v3 algorithm (P6) with expanded placement area to reduce visual overlaps.

Key changes vs P13 (Small):
  - SIGMA_X/Z wider → parts spread over larger area
  - GAP=4 → more clearance between AABBs (fewer visual overlaps)
  - Y_MAX=340 → more vertical room
  - MAX_TRIES=1500, 80 seeds → better search
...
CX, CZ = -100, -130
SIGMA_X, SIGMA_Z = 58, 46       # wider spread than Small (44, 32)
Y_MIN, Y_MAX = 0, 340           # more vertical room
GAP = 4                         # more clearance → fewer visual overlaps
MAX_TRIES = 1500                # more attempts per part
N_SEEDS = 80
SEED_START = 300
...
# ── Multi-seed search ──────────────────────────────────────────────────────────
print("Pocket 14 — Medium size, v3 algorithm, searching seeds...", file=sys.stderr)
best_result = None
best_forced = 9999
...
print(f"X: {min(xs):.0f}→{max(xs):.0f}  ({(max(xs)-min(xs))/20:.1f} studs)",
      file=sys.stderr)
print(f"Y: {min(ys):.0f}→{max(ys):.0f}  ({(max(ys)-min(ys))/8:.0f} plates)",
      file=sys.stderr)
print(f"Z: {min(zs):.0f}→{max(zs):.0f}  ({(max(zs)-min(zs))/20:.1f} studs)",
      file=sys.stderr)

out = "/tmp/pocket14_model.ldr"
